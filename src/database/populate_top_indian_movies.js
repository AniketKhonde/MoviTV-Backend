...
app.get('/fetchTopIndianMovies', async (req, res) => {
    try {
        // Fetch top 100 Indian movies from TMDB API
        const response = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${process.env.TMDB_API_KEY}&language=en-US&region=IN&sort_by=popularity.desc&page=1&with_original_language=hi`);
        ...
        res.status(200).send('Top 100 Indian movies stored successfully.');
        console.log('Top 100 Indian movies stored successfully');
    } catch (error) {
